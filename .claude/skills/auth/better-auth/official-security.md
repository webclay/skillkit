...
description: Security features spanning Better Auth - rate limiting, CSRF protection, session security, trusted origins, secret management, OAuth security, IP tracking, and security auditing.
...
Each plugin can optionally have its own configuration to adjust rate-limit rules for a given endpoint.
